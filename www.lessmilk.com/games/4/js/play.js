...
Game.Play.prototype = {
	...
	create: function () {
	    ...
	    this.labelScore = game.add.text(30, 30, 'score: 0', { font: '16px Arial', fill: '#fff' });
	    this.labelBestScore = game.add.text(30, 50, 'best: 0', { font: '16px Arial', fill: '#fff' });
	    this.labelTutorial = game.add.text(w/2+0.5, h/2-20+0.5, '1- arrow keys to move\n2- take coins for score\n3- jump on penguins to kill', { font: '18px Arial', fill: '#fff' });
	    ...
	    this.coinPosition = [];
	    this.coinPosition[0] = [[150, h/2], [w-150, h/2], [50, h-100], [w-50, h-100]];
	    this.coinPosition[1] = this.coinPosition[0].concat([[60, 100], [w-60, 100]]);
	    this.coinPosition[2] = this.coinPosition[1].concat([[130, 50], [w-130, 50], [200, h-60], [w-200, h-60]]);
		this.coinX = 0;
	...
	enemyDie: function(enemy, killall) {
		...
		tmp.onCompleteCallback(function(){enemy.kill();});
		enemy.alive = false;
	...
	playerDie: function() {
		if (this.player.alive) {
			...
			var tmp = game.add.tween(this.player).to({y : h+10}, 500, Phaser.Easing.Linear.None).start();
			game.add.tween(this.player).to({angle : 360}, 500, Phaser.Easing.Linear.None).start();
			tmp.onComplete.add(this.playerInit, this);
			this.updateBestScore();
			this.enemies.forEachAlive(function(e){this.enemyDie(e, true)}, this);
	...
	updateBestScore: function(){
		if (this.bestScore < this.score) {
			...
			this.game.add.tween(this.labelBestScore.scale).to({x : 1, y : 1}, 300, Phaser.Easing.Linear.None).start();
		}
